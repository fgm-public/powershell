function BackUp-VM{
    ...
    function Enumerate-Items{
        ...
        $items = (Get-ChildItem $BackUpList |
    ...
    function Lull-VM{
        ...
        while ($deal -ne 'q'){
            ...
            if ($answer -eq 'y'){
                ...
                Get-Job |
                    Where-Object -Property Command -Like "*$VM*" |
                        Wait-Job

                if ((Get-VM |
                        Where-Object -Property Name -eq $VM).State -eq "off")
                {
    ...
    function Attach-Disk{
        ...
        if ($Deattach){
            $VMStorages = Get-VMHardDiskDrive -VMName $VMName |
    ...
    function Handle-BackUp{
        ...
        if ($VMName){
            if (Test-Path -Path $BackUpDestination){
                foreach ($VM in $VMName){
                    if ((Get-VM |
                            Where-Object -Property Name -eq $VM).State -eq "Running"){
                        Lull-VM
                    }
                    Start-BackUp
                }
